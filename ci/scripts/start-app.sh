...
curl -X PUT -H "Content-Type: text/plain;charset=UTF-8" -H "api-key: $DASHBOARD_API_KEY" --data-binary @$COMPOSE_FILE "http://$DASHBOARD/api/v2/apps/$APPLICATION_NAME/$INSTANCE_NAME/files/dockerCompose"
...
echo curl -sS -XPUT  -H 'Content-Type: application/json' -H "api-key: $DASHBOARD_API_KEY" \
     ...
     "http://$DASHBOARD/api/v2/instances/$INSTANCE_NAME"
...
a=0
TIMEOUT=200
INSTANCE_STATUS=$(curl -sS -XGET http://$DASHBOARD/api/v2/instances/${INSTANCE_NAME}?api-key=$DASHBOARD_API_KEY)
